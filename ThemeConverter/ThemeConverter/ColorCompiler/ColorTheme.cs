...
internal class ColorTheme
{
    ...
    public ColorTheme(Guid themeId) => ThemeId = themeId;

    public Guid ThemeId { get; private set; }

    public bool IsBuiltInTheme { get; set; }
    ...
    {
        get { return _colors = _colors ?? new ColorEntryCollection(this); }
    }

    public IDictionary<ColorName, ColorEntry> Index
    {
        get { return _index = _index ?? new Dictionary<ColorName, ColorEntry>(); }
    }

    private class ColorEntryCollection : OwnershipCollection<ColorEntry>
    {
        private readonly ColorTheme _theme;

        public ColorEntryCollection(ColorTheme theme) => _theme = theme;

        protected override void TakeOwnership(ColorEntry item)
        {
            if (item.Theme != null) throw new InvalidOperationException("Color entry can only belong to one theme");
