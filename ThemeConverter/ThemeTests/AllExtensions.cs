...
internal static class AllExtensions
{
    ...
    public static AutomationElement GetSolutionExplorerPane(this Application app)
    {
        return Retry.WhileNull(() =>
                                   app.GetMainVSWindow().FindFirstDescendant(
                                       new AndCondition(
                                           ConditionFactory.ByControlType(ControlType.Pane),
    ...
    {
        var mainWindow           = app.GetMainVSWindow();
        var windowMenu           = mainWindow.FindFirstDescendant(cf => cf.ByName("Window").And(cf.ByControlType(ControlType.MenuItem))).AsMenuItem();
    ...
    private static Window GetWindow(Application app, string automationId, TimeSpan? timeout = null)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                if (mainWindow.ControlType == ControlType.Window && mainWindow.AutomationId == automationId)
                {
                    break;
                }
    ...
    {
        return Retry.WhileNull(() => app.GetMainVSWindow().ModalWindows.Where(tlw => tlw.Name == "Options").Single()).Result.AsWindow();
    }
